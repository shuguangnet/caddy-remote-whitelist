...
1. Locate the active Caddyfile and all imported files. Confirm the service name, Caddy binary, and effective config path from systemd before editing.
...
8. Install this repository's updater with the chosen HTTPS URL and interval. Never place a secret token directly in a world-readable Caddyfile.
9. Run `caddy fmt --diff`, `caddy validate --config <path>`, reload Caddy, and independently test one allowed and one denied client path when feasible.
